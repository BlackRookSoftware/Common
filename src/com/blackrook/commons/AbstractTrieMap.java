	...
	extends AbstractTrie<ObjectPair<K, V>, S> implements AbstractMap<K, V>
{
	 ...
	 */
	public int getPreviousValuesByKey(K key, List<V> out)
	{
		return getPreviousValuesByKey(key, out, out.size());
	}
	 ...
	 * @return the amount of items returned into the list.
	 */
	public int getPreviousValuesByKey(K key, List<V> out, int startOffset)
	{
	 ...
	 * @return the amount of items returned into the list.
	 */
	public int getDescendantValuesByKey(K key, List<V> out)
	{
		return getDescendantValuesByKey(key, out, out.size());
	}
	 ...
	 * @return the amount of items returned into the list.
	 */
	public int getDescendantValuesByKey(K key, List<V> out, int startOffset)
	{
	 ...
	 */
	public V getValueWithRemainderByKey(K key, List<S> out, int startOffset)
	{
		Result<ObjectPair<K, V>, S> result = searchByKey(key, true, false);
		...
		{
			for (int i = result.getMoveCount(); i < result.getSegments().length; i++)
				out.replace(startOffset + (i - result.getMoveCount()), result.getSegments()[i]);
