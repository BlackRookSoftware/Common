 ...
 */
public class CustomWaveForm implements WaveFormType
{
	...
	public CustomWaveForm(Random random, double amplitude, int sampleCount, InterpolationType interpolationType)
	{
		...
		this.interpolationType = interpolationType;
		for (int i = 0; i < samples.length; i++)
			samples[i] = RMath.randDoubleN(random) * amplitude;
	}
	
	/**
	 * Creates a new waveform with sample values taken from an existing waveform.
	 * @param waveForm the waveform to use as a basis for this new one.
	 * @param amplitude the desired wave amplitude.
	 * @param sampleCount the number of discrete samples. 
	 * @param interpolationType the type of interpolation to use for sampling values between the discrete samples.
	 */
	public CustomWaveForm(WaveFormType waveForm, double amplitude, int sampleCount, InterpolationType interpolationType)
	{
		this.amplitude = amplitude;
		samples = new double[sampleCount];
		sampleIncrement = 1.0 / samples.length;
		this.interpolationType = interpolationType;
		for (int i = 0; i < samples.length; i++)
			samples[i] = waveForm.getSample(i * sampleIncrement);
	}
	...
	/**
	 * Adds a waveform to this waveform, potentially replacing all of
	 * the existing sample values in the wave. The resultant sample values will
	 * be clamped into this wave's amplitude. This does NOT alter the current amplitude.
	 * @param source the wave to add to this one.
	 */
	public void modulateInline(WaveFormType source)
	{
		double amp = getAmplitude();
		for (int i = 0; i < samples.length; i++)
			samples[i] = RMath.clampValue(samples[i]*source.getSample(sampleIncrement*i), -amp, amp);
	}
	...
	public void modulateInline(double value)
	{
		double amp = getAmplitude();
		for (int i = 0; i < samples.length; i++)
			samples[i] = RMath.clampValue(samples[i]*value, -amp, amp);
	}
	
	/**
	 * Adds a waveform to this waveform, potentially replacing all of
	 * the existing sample values in the wave. The resultant sample values will
	 * be clamped into this wave's amplitude.
	 * @param source the wave to add to this one.
	 */
	public void addInline(WaveFormType source)
	{
		double amp = getAmplitude();
		for (int i = 0; i < samples.length; i++)
			samples[i] = RMath.clampValue(samples[i]+source.getSample(sampleIncrement*i), -amp, amp);
	 ...
	 * wave. The number of samples and interpolation type of the resultant wave
	...
	/**
	 * Adds a waveform to this waveform and returns the contents as a new wave.
	 * The number of samples and interpolation type of the resultant wave
	 * is dependent on the number of samples and the interpolation type in this
	 * wave. The resultant sample values are clamped into this wave's amplitude.
	 * @param source the wave to add to this one.
	 * @return a new CustomWave that is the source wave plus this one.
	...
	{
		time = RMath.wrapValue(time, 0.0, 1.0);
		...
		switch (interpolationType)
		{
			...
			case LINEAR:
			{
				double v2 = samples[RMath.wrapValue((int)Math.ceil(spos), 0, samples.length)];
				double interp = (time % sampleIncrement) / sampleIncrement;
				return RMath.linearInterpolate(interp, v1, v2);
			}
			case COSINE:
			{
				double v2 = samples[RMath.wrapValue((int)Math.ceil(spos), 0, samples.length)];
				double interp = (time % sampleIncrement) / sampleIncrement;
				return RMath.cosineInterpolate(interp, v1, v2);
			}
			case CUBIC:
			{
				double v2 = samples[RMath.wrapValue((int)Math.ceil(spos), 0, samples.length)];
				double v0 = samples[RMath.wrapValue((int)Math.floor(spos - 1.0), 0, samples.length)];
				double v3 = samples[RMath.wrapValue((int)Math.ceil(spos + 1.0), 0, samples.length)];
				double interp = (time % sampleIncrement) / sampleIncrement;
				return RMath.cubicInterpolate(interp, v0, v1, v2, v3);
			}
		}
	}
